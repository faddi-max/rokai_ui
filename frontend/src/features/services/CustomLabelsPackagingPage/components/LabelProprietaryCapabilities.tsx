...
const DEFAULT_CAPABILITIES: CapabilityItemData[] = [
  { number: "01", label: "Premium Woven & Satin Labeling",paragraph:"Custom neck and waist labels engineered for comfort and long-term durability." },
  { number: "02", label: "Anatomical Care-Tag Placement",paragraph:"Strategically placed labels that comply with international regulations without causing skin irritation." },
  { number: "03", label: "High-Impact Retail Packaging", paragraph:"Custom Gi bags, rash guard pouches, and shipping mailers that feature your academy’screst." },
  { number: "04", label: "Smart-Tag & QR Integration",paragraph:"Modernize your gear with QR-coded hangtags that link directly to your academy’scurriculum or shop." },
];

function CapabilityRow({ number, label, paragraph}: CapabilityItemData) {
  return (
    <div className="flex h-auto items-center gap-4 border-b border-[#303030]">
      <span className="font-space-grotesk text-[11px] font-bold text-[#E51B24]">{number}</span>
      <div className="flex flex-col mt-2">
          <span className="font-space-grotesk text-[15px] font-normal leading-[18px] text-white">
        {label}
      </span>
      <p className="font-space-grotesk py-4 text-[13px] font-normal leading-[27px] tracking-[0%] text-[#BDBDBD]">
              {paragraph}
            </p>
      </div>
...
export default function LabelProprietaryCapabilities({
  eyebrow = " Proprietary Packaging Capabilities",
  title = "One manufacturing",
  highlight = "partner Complete control.",
  headerDescription = "Packaging expertise and production support to create a polished presentation around your products and brand identity.",
